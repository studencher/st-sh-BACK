...
export class CloudService implements ICloudService {
  ...
  async generatePreSignUrl(data: IClientRequestData): Promise<ServiceResponse> {
    try {
      ...
      const preSignedUrl: string = await this.cloudBucketAdapter.getSignedUrl(
        fileName,
        data.action
      );
  ...
  async getBucketBilling() {
    ...
    const billing: number = await this.cloudBucketAdapter.getBucketBilling(
      region
    );
    return billing;
  }
  ...
  async getActivitiesVideosOnCloud() {
    return this.cloudBucketAdapter.getFileNamesFromBucketByPrefix(
      Constants.CLOUD_ACTIVITIES_VIDEOS_BUCKET_PREFIX
    );
  }
}
