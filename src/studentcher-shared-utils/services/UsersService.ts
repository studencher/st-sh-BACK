...
} from '../entities/userActivityMetaData';
import { IUserActivityVideoStatus } from '../entities/userActivityVideoStatus';

export interface IUsersService {
  getUsers(
    data: IClientRequestData
  ): Promise<ServiceResponse<{ users: IUserDTO[]; roles: Role[] }>>;
  getUser(data: IClientRequestData): Promise<ServiceResponse<IUserDTO>>;
  addUser(
    data: IClientRequestData
  ): Promise<ServiceResponse<{ user: IUserDTO }>>;
  editUser(
    data: IClientRequestData
  ): Promise<ServiceResponse<{ user: IUserDTO }>>;
  deleteUsers(
    data: IClientRequestData
  ): Promise<ServiceResponse<{ users: IUserDTO[] }>>;
  getPersonalZone(
    data: IClientRequestData
  ): Promise<ServiceResponse<{ privateZone: IUserPrivateZone }>>;
  addUserActivity(data: IClientRequestData): Promise<ServiceResponse<{}>>;
  getUsersCloudUsage(): Promise<
    ServiceResponse<{ userCloudUsages: any; billing: any }>
  >;
  addUserActivityVideoStatus(
    data: IClientRequestData
  ): Promise<
    ServiceResponse<{ userActivityVideoStatus: IUserActivityVideoStatus }>
  >;
}
export class UsersService implements IUsersService {
  ...
  private static validateUserFields(data: IClientRequestData) {
    if (data.id == null) throw new CustomError("User's id must be provided.");
    if (data.password != null) {
      const { result, message } = Validations.isPasswordValid(data.password);
      if (!result) throw new CustomError(message);
    }
    if (data.phoneNumber != null) {
      const { result, message } = Validations.isPhoneNumberValid(
        data.phoneNumber
      );
      if (!result) throw new CustomError(message);
    }
  ...
  async addUser(data: IClientRequestData): Promise<ServiceResponse> {
    try {
      const { result, message } = Validations.areFieldsProvided(
        ['password', 'name', 'roleId', 'discordUserId'],
        data
      );
      if (!result) return { err: new CustomError(message) };
      UsersService.validateUserFields(data);
      data.hashedPassword = await UsersService.encryptionHandler.hash(
        data.password,
        12
      );
      ...
      if (err.constraint === 'users_pkey')
        return {
          err: new CustomError('Email already registered in the system.'),
        };
      return { err };
    }
  }
  ...
  async getPersonalZone(data: IClientRequestData): Promise<ServiceResponse> {
    try {
      ...
      privateZoneData.allActivities.reverse()
      ...
      for (
        ...
        const totalVideos = privateZoneData?.allActivities[activityIndex].videos.length || 0;
        const signedVideos = [];
        for (let i = 0; i < totalVideos; i++) {
          const videoData =
            privateZoneData.allActivities[activityIndex].videos[i];
          const { response, err } = await this.cloudService.generatePreSignUrl({
            fileName: videoData.fileName,
            action: Constants.CLOUD_STORAGE_PRE_SIGNED_URL_READ_ACTION,
          });
          if (err != null) return { err };
          delete videoData.fileName;
          signedVideos.push({
            ...videoData,
            srcUrl: response.data.preSignedUrl,
          });
        }
        privateZoneData.allActivities[activityIndex] = {
          ...privateZoneData.allActivities[activityIndex],
          videos: signedVideos,
        };
      }
      return {
        response: new ApiResponse(true, { privateZone: privateZoneData }),
      };
    } catch (err) {
      return { err };
    }
  ...
  async addUserActivity(data: IClientRequestData): Promise<ServiceResponse> {
    try {
      ...
      if (err.constraint === 'user_activity_history_pkey')
        return {
          err: new CustomError(
            '(user_id, plan_id, activity_id) already monitored in the system'
          ),
        };
      return { err };
    }
  ...
  async addUserActivityVideoStatus(
    data: IClientRequestData
  ): Promise<ServiceResponse> {
    try {
      const { result, message } = Validations.areFieldsProvided(
        ['userId', 'planId', 'activityId', 'videoIndex'],
        data
      );
      if (!result) return { err: new CustomError(message) };
      const userActivityVideoStatus: IUserActivityVideoStatus =
        await this.userRepository.addUserActivityVideo(data);
  ...
  private getUsersEventsByUsersMetaDataList(
    userActivityMetaDataList: UserActivityMetaData[]
  ): Record<string, IVideoMetaData[]> {
    const eventsByUserActivity: Record<string, IVideoMetaData[]> = {};

    for (const activityMetaData of userActivityMetaDataList) {
      const { userId, planId, activityId, metaData, timestamp, videoIndex } =
        activityMetaData;
  ...
  private analyzeAndExtractUsersEvents(
    usersEventsIndex: Record<string, IVideoMetaData[]>
  ) {
  ...
  private getCalculatedUsersUsage(
    usersEventsIndex: Record<string, IVideoMetaData[]>
  ): IUserCloudUsage[] {
    const userUsages: IUserCloudUsage[] = [];
    const { usersUsageIndex, allUsersTotalDuration, allUsersEventsCounter } =
      this.analyzeAndExtractUsersEvents(usersEventsIndex);
  ...
  async getUsersCloudUsage(): Promise<ServiceResponse> {
    try {
      const userActivityMetaDataList =
        await this.userRepository.finDUsersMetaData();
      const usersEventsIndex: Record<string, IVideoMetaData[]> =
        this.getUsersEventsByUsersMetaDataList(userActivityMetaDataList);
      const userCloudUsages: IUserCloudUsage[] =
        this.getCalculatedUsersUsage(usersEventsIndex);
