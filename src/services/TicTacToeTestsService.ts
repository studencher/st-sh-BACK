...
export class TicTacToeTestsService {
    private logger: Logger;
    private dotNetProjectTestsService: DotNetProjectTestsService;

    constructor(logger: Logger, dotNetProjectTestsService: DotNetProjectTestsService) {
        this.logger = logger;
        this.dotNetProjectTestsService = dotNetProjectTestsService;
    }
    private static getUsingJsonPartial(): string {return "Newtonsoft.Json"; }
    private static getOldProgramMainSignature(): string{ return "static int Main"; }
    private static getNewProgramMainSignature(): string{ return "static int TaskMain"; }
    private static getGetUserInputRegex(): RegExp {
        return /static string getUserInput\(\)[^{]*{[^}]*}/g;
    }
    private static getPrintBoardRegex(): RegExp {
        return  /static void printBoard\(char\[\] board\)\s*[^{]*\{[^}]*\}/g;
    }
    private static getGetInitializedBoardRegex(): RegExp {
        return /static char\[\]\[\] getInitializedBoard\(\)[^{]*{[^}]*}/g;
    }

    private static getProgramClassRegex(): RegExp {
        return  /^[\s\n]*class\s+Program\s*({\s*)?/m;
    }

    private static getGetInitializedTurnCounterRegex(): RegExp {
        return /static int getInitializedTurnCounter\(\)[^{]*{[^}]*}/g;
    }

    private static getNewGetUserInputForAutoTest(): string {
    ...
    private static getNewPrintBoardForShadowTest(): string {
    ...
    private static getNewGetUserInputForShadowTest(): string {
        return `static string getUserInput(){
    ...
    private static getNewGetInitializedBoardFunction(): string {
    ...
    private static getNewGetInitializedTurnCounterFunction(): string {
    ...
    private static getNewMainHelpersClassesForAutoTest(): string{
    ...
    private static getNewMainHelpersClassesForShadowTest(): string{
        return `class Program
        {   
    ...
    public async updateFileWithAutoTests(DOTNET_FILE_PATH: string, NEW_DOTNET_FILE_PATH: string ): Promise<void> {
    ...
    public async updateFileWithShadowTests(DOTNET_FILE_PATH: string, NEW_DOTNET_FILE_PATH: string ): Promise<void> {
        try {
            this.logger.info(`Editing file: ${DOTNET_FILE_PATH}`)
            const fileText = await fs.promises.readFile(DOTNET_FILE_PATH, 'utf8');
            const usingJsonPartial = TicTacToeTestsService.getUsingJsonPartial();
...
export default new TicTacToeTestsService(logger, dotNetProjectTestsService);



import * as os from 'os';
import * as path from 'path';
import dotNetProjectTestsService, {DotNetProjectTestsService} from "./DotNetProjectTestsService";
// import * as fs from 'fs';
...
const service = new TicTacToeTestsService(logger, dotNetProjectTestsService);
